...
const UpdateStaff: React.FC = () => {
  ...
  const schema = yup.object().shape({
    ...
    staffPhoneNumber: yup.string().required("Số điện thoại không được để trống").matches(/^[0-9]+$/, "Số điện thoại chỉ được chứa số").min(10, "Số điện thoại phải có ít nhất 10 số"),
    staffDob: yup.string().required("Ngày sinh không được để trống").test("is-18", "Nhân viên phải đủ 18 tuổi trở lên", (value) => !!value && differenceInYears(new Date(), new Date(value)) >= 18),
    staffGender: yup.string().required("Giới tính phải chọn"),
    staffCccd: yup.string().required("Số CCCD không được để trống").matches(/^[0-9]+$/, "CCCD chỉ được chứa số").length(12, "CCCD phải có 12 số"),
  });
